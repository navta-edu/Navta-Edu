...
export default function RewardsPage() {
  ...
  const handleRedeem = async (rewardId, costCoins, title) => {
    if (profile && profile.coins < costCoins) {
      alert(
        `Insufficient coins. This reward costs ${costCoins} coins, but you only have ${profile.coins} coins.`
      );
      return;
    }
    ...
    if (
      !window.confirm(
        `Are you sure you want to spend ${costCoins} coins to redeem '${title}'?`
      )
    ) {
      return;
    }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">
            Redeem accumulated study coins for real badges, discount coupons,
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {rewards.map((rew) => {
              const hasEnough =
                profile && profile.coins >= rew.costCoins;
        ...
        <div className="space-y-6">
          ...
          <Card
            title="Leaderboard"
            subtitle="Top students ranked by NAVTA Coins"
          >
            <div className="mt-4 space-y-4">
              {leaderboard.map((student) => {
                const isCurrentStudent =
                  student.userId &&
                  user?._id &&
                  String(student.userId) === String(user._id);

                return (
                  <div
                    key={student.userId || `${student.rank}-${student.name}`}
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span
                        className={`w-6 h-6 flex items-center justify-center rounded-xl font-black text-xs ${
                          student.rank === 1
                      ...
                      <div>
                        <h4 className="text-xs font-bold text-slate-800 dark:text-white truncate max-w-[110px]">
                          {student.name}
                        </h4>

                        <p className="text-[9px] text-slate-400 mt-0.5">
                          Level {student.level} • {student.badgesCount} Badges
                        </p>
                      </div>
                    </div>

                    <div className="flex items-center gap-1 text-right text-yellow-600 dark:text-yellow-400">
